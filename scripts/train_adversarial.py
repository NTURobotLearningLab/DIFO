...
from difo import difo as difo_algo
from difo.baselines import (
    airl as airl_algo,
    depo as depo_algo,
    gail as gail_algo,
    ot as ot_algo,
...
from envs.wrappers import PixelObservationWrapper
from imitation.algorithms.adversarial import common
from imitation.data import rollout
from imitation.policies import serialize
from scripts.config.train_adversarial import train_adversarial_ex
...
logger = logging.getLogger("scripts.train_adversarial")
...
def save(trainer: common.AdversarialTrainer, save_path: pathlib.Path):
    ...
    th.save(trainer._gen_replay_buffer.sample(2048), save_path / "replay_buffer.pt")
...
def train_adversarial(
    ...
    with environment.make_venv(  # type: ignore[wrong-keyword-args]
        ...
        reward_net = reward.make_reward_net(venv, expert_trajs=expert_trajs)
        ...
        for k in ("shared", "airl", "gail", "difo"):
...
@train_adversarial_ex.command
def difo():
    return train_adversarial(algo_cls=difo_algo.DIFO)


@train_adversarial_ex.command
def wail():
    return train_adversarial(algo_cls=wail_algo.WAIL)


@train_adversarial_ex.command
def depo():
    return train_adversarial(algo_cls=depo_algo.DePO)


@train_adversarial_ex.command
def ot():
    return train_adversarial(algo_cls=ot_algo.OT)
