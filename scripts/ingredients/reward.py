...
def config_hook(config, command_name, logger):
    ...
    if "condition" in config["reward"]["net_cls"].__name__.lower():
        logger.info("Adding use states to condition reward network.")
        res["net_kwargs"] = {
            "use_state": True,
            "use_action": False,
            "use_next_state": True,
...
def make_reward_net(
    ...
    if issubclass(net_cls, reward_nets.RewardEnsemble):
        ...
        reward_net = _make_reward_net(venv, net_cls, net_kwargs, normalize_output_layer)
    if hasattr(net_cls, "set_demonstrations"):
        reward_net.set_demonstrations(expert_trajs)
    logger.info(f"Reward network summary:\n {reward_net}")
    return reward_net
