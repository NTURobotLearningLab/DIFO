...
from difo.baselines.bco import compute_ac_loss
...
from .decoupled_policies import SACPolicy
...
def compute_train_stats(
    ...
    with th.no_grad():
        ...
        if use_logits:
            ...
            bin_is_generated_pred = disc_logits_expert_is_high < 0.5
    ...
    results = {
        ...
        "disc_logit_expert": float(
            th.mean(disc_logits_expert_is_high[bin_is_expert_true])
        ),
        "disc_logit_gen": float(
            th.mean(disc_logits_expert_is_high[bin_is_generated_true])
        ),
...
class DePO(base.DemonstrationAlgorithm[types.Transitions]):
    ...
    def __init__(
        ...
        *,
        ...
        demo_batch_size: int,
        venv: vec_env.VecEnv,
        gen_algo: SACDecoupled,
        reward_net: reward_nets.RewardNet,
        disc_gen_ratio: float = 1.0,
        n_combined_batch: int = 1,
        log_dir: types.AnyPath = "output/",
        disc_opt_cls: Type[th.optim.Optimizer] = th.optim.Adam,
        disc_opt_kwargs: Optional[Mapping] = None,
        disc_grad_clip: Optional[float] = None,
        gen_train_timesteps: Optional[int] = None,
        gen_replay_buffer_capacity: Optional[int] = None,
        custom_logger: Optional[logger.HierarchicalLogger] = None,
        logger_dump_interval: int = 1,
        init_tensorboard: bool = False,
        init_tensorboard_graph: bool = False,
        debug_use_ground_truth: bool = False,
        allow_variable_horizon: bool = False,
        inv_steps: int = 10,
        inv_batch_size: int = 256,
        inv_lr: float = 3e-4,
        inv_train_interval: int = 10,
        ...
        self._global_step = 0
        self._disc_step = 0
        self.disc_gen_ratio = disc_gen_ratio
        self.n_combined_batch = n_combined_batch
        self._reward_net_output_logits = True
        ...
        self._disc_grad_clip = disc_grad_clip
        ...
        self.logger_dump_interval = logger_dump_interval
        ...
        if isinstance(self.gen_algo, off_policy_algorithm.OffPolicyAlgorithm):
            self.gen_algo.replay_buffer.reward_fn = self.reward_train.predict_processed

        if gen_train_timesteps is None:
            gen_algo_env = self.gen_algo.get_env()
            assert gen_algo_env is not None
            self.gen_train_timesteps = gen_algo_env.num_envs * n_combined_batch
            self.n_disc_updates_per_round = n_combined_batch
            if isinstance(self.gen_algo, on_policy_algorithm.OnPolicyAlgorithm):
                self.gen_train_timesteps *= self.gen_algo.n_steps
                self.n_disc_updates_per_round *= self.gen_algo.n_steps
        else:
            UserWarning(
                "Passing `gen_train_timesteps` is deprecated. "
                "Use `n_combined_batch` instead.",
            )
            self.gen_train_timesteps = gen_train_timesteps
        if isinstance(self.gen_algo, off_policy_algorithm.OffPolicyAlgorithm):
            self.gen_algo.gradient_steps = n_combined_batch
            self.gen_algo.train_freq = n_combined_batch
            self.gen_algo._convert_train_freq()

        self.n_disc_updates_per_round *= self.disc_gen_ratio
        assert (
            self.n_disc_updates_per_round
        ) % 1 == 0, "Number of discriminator updates per round must be an integer."
        self.n_disc_updates_per_round = int(self.n_disc_updates_per_round)
        print(
            f"{self.gen_train_timesteps=}, {self.n_combined_batch=}, {self.n_disc_updates_per_round=}"
        )
        ...
        self.inv_train_interval = inv_train_interval
        self.inv_opt = th.optim.Adam(
            self.policy.actor.inverse_dynamic_model.parameters(), lr=inv_lr
        )

    @property
    def policy(self) -> SACPolicy:
    ...
    def calc_loss(
    ...
    ) -> Tuple[th.Tensor, th.Tensor, Optional[Mapping]]:
        """Compute the loss of the discriminator for each state-action sample.
        ...
        Args:
            ...
            log_policy_act_prob: log probability of generator policy taking
                `action` at time t.
            labels_expert_is_one: binary expert label for each sample.
    ...
    def set_demonstrations(self, demonstrations: base.AnyTransitions) -> None:
        ...
        while True:
            try:
                ...
                self.combined_round *= 2
                print(f"Increasing combined round to {self.combined_round}")
        self._endless_expert_iterator = util.endless_iter(self._demo_data_loader)
        self.gen_algo.set_demonstrations(demonstrations)
    ...
    def train_disc(
        ...
        *,
        ...
        with self.logger.accumulate_means("disc"):
            ...
            # compute loss
            for _ in range(self.combined_round):
                batch_iter = self._make_disc_train_batches(
                    gen_samples=gen_samples,
                    expert_samples=expert_samples,
                )
                for batch in batch_iter:
                    loss, disc_logits, info = self.calc_loss(
                        batch["state"],
                        batch["action"],
                        batch["next_state"],
                        batch["done"],
                    ...
                    assert len(batch["state"]) == 2 * self.demo_batch_size

                    # do gradient step
                    self._disc_opt.zero_grad()
                    loss.backward()
                    if self._disc_grad_clip is not None:
                        th.nn.utils.clip_grad_norm_(
                            self._reward_net.parameters(), max_norm=self._disc_grad_clip
                        )
                    self._disc_opt.step()
                    self._disc_step += 1
                    ...
                    with th.no_grad():
                        train_stats = compute_train_stats(
                            ...
                            batch["labels_expert_is_one"],
                            loss,
                            use_logits=self._reward_net_output_logits,
                            info=info,
                        )
    ...
    def train_inv(self):
        infer_ac_losses = []
        for i in range(self.inv_steps):
            gen_samples = self._gen_replay_buffer.sample(self.inv_batch_size)

            obs = (
                util.safe_to_tensor(gen_samples.obs)
                .to(self.gen_algo.device)
                .to(th.float32)
            )
            next_obs = (
                util.safe_to_tensor(gen_samples.next_obs)
                .to(self.gen_algo.device)
                .to(th.float32)
            )
            actions = (
                util.safe_to_tensor(gen_samples.acts)
                .to(self.gen_algo.device)
                .to(th.float32)
            )

            pred_action = self.policy.actor.inverse_dynamic_model(
            ...
            self.logger.record("inv/infer_action_loss", loss.item())
    ...
    def train(
        ...
        for r in tqdm.tqdm(range(0, n_rounds), desc="round", dynamic_ncols=True):
            self.train_gen(self.gen_train_timesteps)
            with networks.training(self.reward_train):
                # switch to training mode (affects dropout, normalization)
                self.train_disc()

            if r % self.inv_train_interval == 0:
                self.train_inv()

            if callback:
                callback(r)

            if r % self.logger_dump_interval == 0:
    ...
    def _get_log_policy_act_prob(
        ...
        elif isinstance(self.policy, SACPolicy):
    ...
    def _make_disc_train_batches(
        ...
        *,
        ...
        """
        combined_batch_size = (
            self.demo_batch_size * self.n_disc_updates_per_round // self.combined_round
        )
        ...
        if gen_samples is None:
            ...
            gen_samples_dataclass = self._gen_replay_buffer.sample(combined_batch_size)
            gen_samples = types.dataclass_quick_asdict(gen_samples_dataclass)

        if not (
            len(gen_samples["obs"]) == len(expert_samples["obs"]) == combined_batch_size
        ):
            raise ValueError(
                ...
                f"demo_batch_size={combined_batch_size})",
            )
        ...
        # Check dimensions.
        assert combined_batch_size == len(expert_samples["acts"])
        assert combined_batch_size == len(expert_samples["next_obs"])
        assert combined_batch_size == len(gen_samples["acts"])
        assert combined_batch_size == len(gen_samples["next_obs"])

        for start in range(0, combined_batch_size, self.demo_batch_size):
            ...
            labels_expert_is_one = np.concatenate(
                [
                    np.ones(self.demo_batch_size, dtype=int),
                    np.zeros(self.demo_batch_size, dtype=int),
                ],
            )

            # Calculate generator-policy log probabilities.
            log_policy_act_prob = None  # Not used
            #### Calculate generator-policy log probabilities. ####
            if getattr(self, "_calc_log_policy_act_prob", False):
                with th.no_grad():
                    ...
                    if log_policy_act_prob is not None:
                        assert len(log_policy_act_prob) == 2 * self.demo_batch_size
                        log_policy_act_prob = log_policy_act_prob.reshape(
                            (2 * self.demo_batch_size,),
                        )
