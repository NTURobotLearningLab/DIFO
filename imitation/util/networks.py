...
class MinMaxNorm(nn.Module):
    """Normalizes input to the range [-1, 1]."""
    ...
    def __init__(self, num_features: int):
        ...
        """
        super().__init__()
        self.register_buffer("min", th.zeros(num_features))
        self.register_buffer("max", th.ones(num_features))
    ...
    def unnormalize(self, x: th.Tensor) -> th.Tensor:
        """Unnormalizes `x` by reversing the normalization process."""
        return 0.5 * (x + 1) * (self.max - self.min) + self.min

    def update_stats(self, batch: th.Tensor) -> None:
        """Update `self.min` and `self.max`."""
        if th.any(self.min > th.min(batch, dim=0).values):
            new_min = th.min(self.min, th.min(batch, dim=0).values)
            self.min = new_min
        if th.any(self.max < th.max(batch, dim=0).values):
            new_max = th.max(self.max, th.max(batch, dim=0).values)
            self.max = new_max

    def set_stats(
        self, mean: th.Tensor, std: th.Tensor, min: th.Tensor, max: th.Tensor
    ) -> None:
        """Set the running mean and variance to the specified values."""
        if not isinstance(min, th.Tensor) or not isinstance(max, th.Tensor):
            min = th.FloatTensor(min).to(self.min.device)
            max = th.FloatTensor(max).to(self.max.device)
...
class BaseNorm(nn.Module, abc.ABC):
    ...
    def unnormalize(self, x: th.Tensor) -> th.Tensor:
        """Unnormalizes `x` by reversing the normalization process."""
        return x * th.sqrt(self.running_var + self.eps) + self.running_mean
    ...
    def set_stats(
        ...
        """Set the running mean and variance to the specified values."""
        if not isinstance(mean, th.Tensor) or not isinstance(std, th.Tensor):
            mean = th.FloatTensor(mean).to(self.running_mean.device)
            std = th.FloatTensor(std).to(self.running_var.device)

        self.running_mean = mean
        self.running_var = std
