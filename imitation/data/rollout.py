    ...
    def add_steps_and_auto_finish(
        ...
        for env_idx, (act, ob, rew, done, info) in zip_iter:
            if done:
                ...
                terminal_observation = types.maybe_wrap_in_dictobs(
                    info["terminal_observation"]
                )
                if ob.shape == terminal_observation.shape:
                    real_ob = terminal_observation
                elif "pixels" in info:
                    real_ob = types.maybe_wrap_in_dictobs(info["pixels"])
                else:
                    raise ValueError(
                        f"Expected terminal observation to have shape {ob.shape}"
                    )
            else:
                real_ob = ob
...
def generate_trajectories(
    ...
    *,
    ...
    wrapped_obs = types.maybe_wrap_in_dictobs(obs)
    if use_pixels:
        frames = np.array(venv.get_images())
        wrapped_obs = frames
    ...
    for trajectory in trajectories:
        ...
        if use_pixels:
            assert real_obs == (n_steps + 1,) + frames.shape[1:]
        else:
...
def rollout_stats(
    ...
    for t in trajectories:
        if t.infos is not None:
            ...
            success = t.infos[-1].get("is_success")
    ...
    if successes:
        out_stats["success_rate"] = float(np.mean(successes))
    ...
    for k, v in out_stats.items():
        assert isinstance(
            v, (int, float)
        ), f"expected int or float, got {type(v)} for {k}"
    return out_stats
...
def flatten_trajectories(
    ...
    Args:
        ...
        next_obs_step: (s_t, s_{t+next_obs_step})
    ...
    for traj in trajectories:
        obs = traj.obs
        parts["obs"].append(obs[:-next_obs_step])
        parts["next_obs"].append(obs[next_obs_step:])

        if next_obs_step != 1:
            length_obs = len(obs[:-next_obs_step])
            act_dim = traj.acts.shape[1]
            parts["acts"].append(np.zeros((length_obs, act_dim)))
            parts["dones"].append(np.zeros(length_obs, dtype=bool))
            parts["infos"].append(np.array([{}] * length_obs))
            continue
